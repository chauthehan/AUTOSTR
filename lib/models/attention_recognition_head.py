...
from lib.utils.labelmaps import dataset_info
...
class AttentionRecognitionHead(nn.Module):
  ...
  def greedy_sample(self, x):
    batch_size = x.size(0)
    all_ones = torch.ones(batch_size)
    all_finished = torch.zeros(batch_size)
    state = torch.zeros(1, batch_size, self.sDim)
    y_prev = torch.zeros((batch_size)).fill_(self.num_classes)  # BOS token
    predicted_ids = []
    for di in range(self.max_len_labels):
      output, state = self.decoder(x, state, y_prev)
      y_prev = torch.argmax(output, dim=1)
      predicted_ids.append(y_prev)
      all_finished = torch.where(y_prev == dataset_info.end_label, all_ones, all_finished)
      if all_finished.sum() == batch_size:
        break
    outputs = torch.stack(predicted_ids, dim=1)  # b, T
    return outputs
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    for i in range(self.max_len_labels):
      ...
      #print('squeezzzz', predecessors.squeeze().type(torch.cuda.LongTensor))
